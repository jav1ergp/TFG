...
def parking_status():
    ...
    available_entry_spots_car = max(0, TOTAL_ENTRY_SPOTS_CAR - occupied_entry_car)
    available_exit_spots_car = max(0, TOTAL_EXIT_SPOTS_CAR - occupied_exit_car)
    
    available_entry_spots_moto = max(0, TOTAL_ENTRY_SPOTS_MOTO - occupied_entry_moto)
    available_exit_spots_moto = max(0, TOTAL_EXIT_SPOTS_MOTO - occupied_exit_moto)
